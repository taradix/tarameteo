...
from tarameteo.msc import MscReading, _parse_features
...
def test_parse_features_count():
    readings = _parse_features(FIXTURE_FEATURES, year=2025, month=1)
...
def test_parse_features_skips_future():
    ...
    assert _parse_features(features, year=9999, month=1) == []


def test_parse_features_empty():
    assert _parse_features([], year=2025, month=1) == []
...
def test_parse_features_invalid_day_skipped(day):
    ...
    assert _parse_features(features, year=2025, month=1) == []
